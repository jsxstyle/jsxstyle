...
class CustomProp<T extends string | number> {
  constructor(
...
export const makeGetPropsFunction = <TCustomPropMap extends CustomPropMap>(
  ...
  const defaultClassNames = defaultStyles
    ...
    : null;
  const customPropClassNames = variants
    ? processCustomPropMap(variants, cache)
    : null;
  ...
  const getProps = <TComponentProps extends Record<string, any>>(
    ...
    return getMakeComponentProps<TCustomPropMap, TComponentProps>(
      'className',
      props,
      defaultClassNames || null,
      customPropClassNames || null
    );
  };
